...
export function LoadVehicleView() {
  ...
  const fetchSheetData = async () => {
    try {
      ...
      if (result.success && result.data) {
        const pending = []
        const history = []
        const processedEntries = new Set() // Track processed entries
        ...
        for (let i = 6; i < result.data.length; i++) {
          ...
          const entryId = `${gateEntryNumber}-${orderNumber}-${truckNumber}-${i}`
  ...
  const openDialog = (entry) => {
    ...
    const formUrl = `https://docs.google.com/forms/d/e/1FAIpQLSd-M2aqroQW1ayDnvZEDznnojF-9jYyuV6n_UP2eCm-Dh4Wfw/viewform?usp=pp_url&entry.213574729=${encodeURIComponent(entry.orderNumber)}&entry.2114644742=${encodeURIComponent(entry.truckNumber)}&entry.1217504241=${encodeURIComponent(entry.customerName)}`;
                ...
                <tbody className="divide-y divide-gray-200">
                  {pendingData.length > 0 ? (
                    pendingData.map((entry, index) => (
                      <tr key={`${entry.gateEntryNumber}-${entry.orderNumber}-${index}`} className="hover:bg-gray-50">
                ...
                <tbody className="divide-y divide-gray-200">
                  {historyData.length > 0 ? (
                    historyData.map((entry, index) => (
                      <tr key={`${entry.gateEntryNumber}-${entry.orderNumber}-${index}-history`} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{entry.orderNumber}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-gray-900">{entry.gateEntryNumber}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-gray-900">{entry.customerName}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-gray-900">{entry.truckNumber}</td>
